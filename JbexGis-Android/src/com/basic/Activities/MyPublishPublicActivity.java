...
import com.basic.Activities.MyPublishDynamciAcvtivity.deletePublishDynamicAsyncTask;
import com.basic.Activities.MyPublishDynamciAcvtivity.setDataAsyncTask;
import com.basic.ImageLoad.ImageOptions;
import com.basic.adapter.myPublishDynamicAdapter;
import com.basic.adapter.myPublishPublicAdapter;
...
public class MyPublishPublicActivity extends ZJBEXBaseActivity implements IXListViewListener {
	...
	private myPublishPublicAdapter mpublishpublicadapter;
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		setContentView(R.layout.activity_my_publish_public);
		
		listmenu=(PullToRefreshSwipeMenuListView) findViewById(R.id.listmypublishpublic);
		intent=getIntent();
	...
	private void initActionBar() {
		...
		backtxt.setOnClickListener(new OnClickListener() {
			...
			public void onClick(View arg0) {
				// TODO 自动生成的方法存根
				MyPublishPublicActivity.this.finish();
			}
		});
	...
	private void initListener() {
		...
		listmenu.setOnMenuItemClickListener(new OnMenuItemClickListener() {
			...
			public void onMenuItemClick(int position, SwipeMenu menu, int index) {
				...
				switch (index) {
				case 0:
					deletePublishpublicAsyncTask delete=new deletePublishpublicAsyncTask(position);
					delete.execute();
					break;
				}
			}
	...
	private void initView() {
		// TODO 自动生成的方法存根
		mpublishpublicadapter=new myPublishPublicAdapter(MyPublicInfoList, this, mImageLoader, options);
		listmenu.setAdapter(mpublishpublicadapter);
	...
	public boolean onCreateOptionsMenu(Menu menu) {
		...
		getMenuInflater().inflate(R.menu.my_publish_public, menu);
		return true;
	}
	...
	public class setDataAsyncTask extends AsyncTask<Integer, Integer, String>{
		...
		//此方法在后台线程执行，完成任务的主要工作，通常需要较长的时间。在执行过程中可以调用publicProgress(Progress…)来更新任务的进度。
			...
			protected String doInBackground(Integer... arg0) {
				...
				MyPublicInfoList=PublicInfoService.getPublicInfoListByUserid(String.valueOf(owneruser.getUser_id()));
				//setData();
		...
		//得到的结果处理操作UI。 此方法在主线程执行，任务执行的结果作为此方法的参数返回
			...
			protected void onPreExecute() {
				...
				if (progressDialog == null){
					progressDialog = CustomProgressDialog.createDialog(MyPublishPublicActivity.this);
	...
	public class deletePublishpublicAsyncTask extends AsyncTask<Integer, Integer, String>{
	     private int position;
	     
	     public deletePublishpublicAsyncTask(int position) {
			super();
		...
		//此方法在后台线程执行，完成任务的主要工作，通常需要较长的时间。在执行过程中可以调用publicProgress(Progress…)来更新任务的进度。
			...
			protected String doInBackground(Integer... arg0) {
				// TODO 自动生成的方法存根
				String flag;
				flag=PublicInfoService.deletePublicinfo(String.valueOf(MyPublicInfoList.get(position).getId()));
				return flag;
			}
	     //相当于Handler 处理UI的方式，在这里面可以使用在doInBackground
		//得到的结果处理操作UI。 此方法在主线程执行，任务执行的结果作为此方法的参数返回
			...
			protected void onPostExecute(String result) {
				...
				if(result.endsWith("true")){
					text="恭喜你！删除成功";
					MyPublicInfoList.remove(position);
					mpublishpublicadapter.onDateChange(MyPublicInfoList);
				}
				else 
					text="删除失败";
				
				Toast.makeText(MyPublishPublicActivity.this, text, Toast.LENGTH_SHORT).show();
